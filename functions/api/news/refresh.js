export async function onRequestGet({ env }) {
  ...
  for(const f of FEEDS){
    try{
      ...
      for(const it of items){
        ...
        list.push({ id, source:f.name, region:f.region, title, link, cover, summary, pub_at: iso(pub) });
      }
    }catch{}
  }
  // 批量 upsert
  if(list.length){
    const stmts = list.map(x=> env.DB.prepare(
  ...
  return new Response(JSON.stringify({ ok:true, list }), { headers:{ "content-type":"application/json" }});

  // helpers
  function pick(seg, tag){
    const re = new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`, "i");
    const m = seg.match(re); if(!m) return "";
    return decode((m[1]||"").replace(/<!\\[CDATA\\[([\\s\\S]*?)\\]\\]>/g,"$1").trim());
  }
  function decode(s){ try{ return s.replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&amp;/g,"&").replace(/&quot;/g,'"').replace(/&#39;/g,"'"); }catch{return s} }
  function text(html){ return (html||"").replace(/<script[\s\S]*?<\/script>/gi,"").replace(/<style[\s\S]*?<\/style>/gi,"").replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim(); }
  ...
  async function sha1(str){ const b=new TextEncoder().encode(str); const h=await crypto.subtle.digest("SHA-1", b); return [...new Uint8Array(h)].map(x=>x.toString(16).padStart(2,"0")).join(""); }
}
